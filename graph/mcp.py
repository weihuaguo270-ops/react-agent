"""
MCP 客户端 — 通过 JSON-RPC over stdio 连接 MCP Server
...
与 hand-written 版 mcp_client.py 逻辑一致，适配 LangChain 工具格式。
"""
...
class MCPClient:
    ...
    def to_langchain_tools(self):
        """
        将 MCP 工具转为 LangChain @tool 风格的函数列表。

        返回: [(tool_name, callable), ...]
        """
        results = []
        ...
        for t in self.tools:
            ...
            def _make_callable(tool_name=name):
                def _fn(**kwargs):
            ...
            results.append((name, _make_callable()))
...
def connect_default_servers():
    """连接默认的 MCP 服务器列表，返回 list[MCPClient]"""
    clients = []
    servers = [
        ["uvx", "mcp-server-time"],
    ]
...
if __name__ == "__main__":
    import sys
    if len(sys.argv) < 2:
        print("用法: python mcp.py uvx mcp-server-time")
        sys.exit(1)
