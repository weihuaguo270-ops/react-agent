...
> 从零手写 ReAct Agent Runtime **理解原理** → LangGraph 框架 **用于实战**。
>
> 项目含两个独立版本：
> - **`src/handwritten_react_agent/`** — 手写版：不依赖框架，纯 Python + LLM API，理解 Agent 最核心机制
> - **`experiments/langgraph/`** — 框架版：LangChain + LangGraph 构建的 14 模块生产级 Agent
>
> 前者是为了"学透"，后者是为了"用起来"。
...
│       ├── rag.py                # RAG 检索增强
│       ├── memory.py             # 语义记忆系统（BGE + LRU）
│       ├── resilience.py         # 容错机制（ToolGuard 工具保护层 + 重试）
│       ├── intent/               # 意图分类
│       │   ├── __init__.py
│       │   └── classifier.py     # IntentClassifier（查询型→回用户 / 生成式→进 Loop）
│       ├── safety/               # 运行时安全
│       │   ├── __init__.py
│       │   ├── permissions.py    # 四级权限（SAFE/NOTIFY/CONFIRM/DENY）
│       │   ├── human_in_the_loop.py # 人工审批 + 重试决策
│       │   └── trace_watch.py    # 执行监控（震荡检测、方向检查）
│       ├── eval.py               # [已弃用] 旧版评测
...
├── notes/                # 开发笔记
├── experiments/
│   ├── langgraph/       # LangChain/LangGraph 全栈版本（14 模块生产级 Agent）
│   │   ├── graph/
│   │   │   ├── agent.py       # 单 Agent StateGraph（集成重试+HITL）
│   │   │   ├── orchestrator.py# 多 Agent 编排（任务分解+Worker执行）
│   │   │   ├── tools.py       # 7 个 @tool 工具
│   │   │   ├── safety.py      # 权限检查 + HITL 审批
│   │   │   ├── resilience.py  # LLM 重试 + 错误分类
